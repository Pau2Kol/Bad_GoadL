<#
.SYNOPSIS
    crypto-fix.ps1 — répare le fournisseur cryptographique Provider Type 24
...
.DESCRIPTION
    Provider Type 24 (PROV_RSA_AES) correspond au fournisseur
    ...
    Exécuté sur dc01 via pypsrp, APRÈS gpo-inheritance.ps1 -Block
    (scripts/30-goad-hardening-fix.sh) : les GPO de durcissement GOAD
    réécrasent ce fix si l'héritage n'est pas bloqué avant.
...
function Remove-CorruptedKeyContainer {
    ...
    # Un conteneur de clé "corrompu" est détecté par une taille de 0 octet
    # (signature la plus courante d'un conteneur tronqué).
    if (-not (Test-Path -Path $Path)) {
