#!/usr/bin/env bash
# 30-goad-hardening-fix.sh — B4+B5 (partie blocage) : établit le tunnel
...
#      ACL MachineKeys)
#
# Le déblocage de l'héritage GPO (étape 4 de la spec §4/B4-B5) est un script
...
# vrai DC Windows durci par GOAD. À l'inverse des scripts réseau (Lot 2), ne
# pas exécuter ce script en conditions réelles sans validation explicite
# préalable de l'opérateur. Attention : même sous --dry-run, get_jumpbox_public_ip
# (lookup de l'IP publique via `az vm list-ip-addresses`) s'exécute réellement
# contre l'abonnement Azure réel (lecture seule, indépendante de $DRY_RUN — cf.
# son commentaire ci-dessous). Le tunnel SSH et les invocations PowerShell,
# eux, sont bien entièrement mockés en --dry-run (aucune connexion SSH/WinRM
# réelle n'est établie tant que --dry-run est actif).
#
...
# apply_hardening_fix — orchestration complète de B4 (blocage GPO + crypto
# fix). Le déblocage GPO (B5, fin) est un script séparé
# (scripts/50-goad-gpo-unblock.sh, cf. spec).
apply_hardening_fix() {
  ...
  log_info "Étape 1/2 : blocage de l'héritage GPO (avant le crypto fix — ordre critique, cf. CHANGELOG)."
